...
class IDMParams:
    ...
    v0: np.ndarray      # 期望速度
    T: np.ndarray       # 安全车头时距
    s0: np.ndarray      # 静止安全距离
    a: np.ndarray       # 最大加速度
    b: np.ndarray       # 舒适减速度
    delta: np.ndarray   # 加速度指数
    length: np.ndarray  # 车长（已加入，建议设为5.0）
    rtime: np.ndarray   # 反应时间 (用于平滑加速度)
...
class EnvState:
    """环境状态"""
    position: np.ndarray      # 车辆位置 (N,)
    velocity: np.ndarray      # 车辆速度 (N,)
    acceleration: np.ndarray  # 当前加速度 (N,)
    target_pos: np.ndarray    # 目标停止位置 (N,)
    params: IDMParams          # 车辆参数
    step_count: int            # 步数
    crashed: bool              # 是否发生碰撞
    front_car_id: np.ndarray  # shape=(N,)
    red_light_pos: float       # 红灯位置
    red_light_state: bool      # 红灯是否为红(True)/绿(False)
    red_light_remaining: float # 红灯剩余时间(秒)
    time_to_vanish: np.ndarray # 每辆车通过红灯的时间，未通过为-1
    acc_stop: np.ndarray      # 停止加速度
    final_acc: np.ndarray     # 最终加速度
# ==========================================
...
def compute_idm_acc(
    ...
    # 没有前车时free_acc
    free_acc = np.where(has_front, free_acc_orig1, free_acc_orig0)

    # 交互项
    delta_v = v - v_front
    s_star = params.s0 + v * params.T + (v * delta_v) / (2.0 * np.sqrt(params.a * params.b))
...
def compute_stopping_acc(
    ...
    req_acc = -(v ** 2) / (2.0 * np.maximum(net_dist, 0.01))

    # 限制最大减速度，防止数值爆炸，但允许紧急制动
    stop_acc = np.maximum(req_acc, -9.0)

    # 如果距离非常近，强制急停（JAX兼容写法）
    stop_acc = np.where(net_dist <= 0, -9.0, stop_acc)
...
def sort_by_target_distance(position: np.ndarray, target_pos: np.ndarray):
    """
    按照位置升序排序，返回排序后的索引。
    位置最大的为头车（索引0），依次类推。
    """
...
class BraxIDMEnv:
    ...
    def reset(self, rng: np.ndarray, init_pos: np.ndarray,init_vel: np.ndarray, params: IDMParams) -> EnvState:
        """初始化环境，所有车辆目标点为1000，支持初始速度设定"""
        target_pos = np.ones(self.num_vehicles) * 300.0
        return EnvState(
            position=init_pos,
            velocity=init_vel,
            ...
            step_count=0,
            crashed=False,
            front_car_id=np.full((self.num_vehicles,), -1, dtype=np.int32),
            red_light_pos=self.red_light_pos,
            red_light_state=True,  # 初始为红灯
            red_light_remaining=self.red_light_duration,
            time_to_vanish=np.full((self.num_vehicles,), -1.0),
            acc_stop=np.zeros(self.num_vehicles),
            final_acc=np.zeros(self.num_vehicles)
        )

  
    def step(self, state: EnvState, action: Optional[np.ndarray] = None, idm_log_list: list = None) -> EnvState:
        """
        单步仿真，返回新状态。
        ...
        front_car_id_sorted = np.full((len(idx),), -1, dtype=np.int32)
        front_car_id_sorted[1:] = idx[:-1]
        # 恢复原始顺序
        front_car_id = np.empty_like(front_car_id_sorted)
        front_car_id[inv_idx] = front_car_id_sorted

        # 红灯逻辑：如果红灯为红，且车辆距离红灯小于一定阈值，则目标点临时设为红灯位置
        red_light_arr = np.ones_like(pos) * state.red_light_pos
        ...
        tgt = np.where(stop_mask, state.red_light_pos, tgt)

        #头车前方无车，gap设为极大，前车速度设为0，头车的idx为0
        #index 0 is the front car of index 1
        pos_front = np.concatenate([np.array([1e9]),pos[0:-1]])
        vel_front = np.concatenate([np.array([0.0]),vel[0:-1]])
        ...
        # 取更强制动（停车优先）
        #target_mask = tgt < 1e8
        final_acc = np.where(stop_mask, np.minimum(acc_idm, acc_stop), acc_idm)
        #final_acc = acc_idm
        # 平滑加速度
        alpha = np.exp(-p_sorted.rtime/10/self.dt)
        smoothed_acc = alpha*final_acc + (1-alpha) * acc
        smoothed_acc = np.clip(smoothed_acc, -9.0, p_sorted.a)
      

        # 积分更新
        new_vel = np.maximum(vel + smoothed_acc * self.dt, 0.0)
        ...
        new_time_to_vanish = np.where((prev_time_to_vanish < 0) & passed_mask, (state.step_count+1)*self.dt, prev_time_to_vanish)
    ...
    def rollout(self, state: EnvState, max_steps: int = 200, idm_log_csv: str = None):
        ...
        for _ in range(max_steps):
            ...
            if state.crashed:
                break
            # 达成目标：所有车都已通过目标点
            arrived = np.all(state.position - state.target_pos > 0.0)
            if arrived:
                break
...
def plot_traj(traj, save_gif=False, gif_path="idm_traj.gif", save_csv=False, csv_path="traj_log.csv", log_detail=False):
    ...
    for idx, state in enumerate(traj):
        ...
        sort_idx = np.argsort(state.position)
        ...
        ax.set_xlim(0, float(np.max(state.target_pos)) + 10)
        ax.set_ylim(-1, 1)
...
def test1():
    """测试二车idm模型 ,看环境仿真效果 """
    params = IDMParams(
        v0=np.array([55.0/3.6, 55.0/3.6]),  # 两辆车的期望速度
        T=np.array([1, 1]),
        s0=np.array([2.0, 2.0]),
        a=np.array([2, 2]),
        b=np.array([6.0, 6]),
        delta=np.array([4.0, 4.0]),
        length=np.array([5.0, 5.0]),
        rtime=np.array([0.02, 0.02])
    )
    env = BraxIDMEnv(num_vehicles=2, dt=0.1, red_light_pos=100.0,red_light_duration=30.0)
    # 初始位置和目标
    init_pos = np.array([50.0/3.6*1+5, 0])  # 车1在后，车0在前.idm适合与稳定的跟车阶段，不是追赶阶段
    init_vel = np.array([50.0/3.6, 50.0/3.6])  # 车1在后，车0在前
