...
# w99参数边界
BASE_BOUND_VEHICLE = [
    (0.5, 2.5),      # 0: CC0 (停车间距 ax) [m]  
    (0.5, 2.5),       # 1: CC1 (车头时距 bx_add) [s]  
    (0.3, 1.5),       # 2: CC2 (跟车变量 bx_mult) [m/s²]  
    (1.0, 3.0),       # 3: CC3 (进入跟车阈值 sdv_fact) [m/s]  
    (0.5, 2.0),       # 4: CC4 (消极跟车阈值 opdv_fact) [m/s]  
    (0.0, 0.8),       # 5: CC5 (积极跟车阈值 opdv_add) [m/s]  
    (0.0, 0.5),       # 6: CC6 (速度振动) [无量纲]  
    (0.5, 2.0),       # 7: CC7 (减速度波动) [m/s²]  
    (1.5, 3.0),       # 8: CC8 (启动加速度) [m/s²]  
    (0.8, 1.8)        # 9: CC9 (高速加速度) [m/s²]
]
