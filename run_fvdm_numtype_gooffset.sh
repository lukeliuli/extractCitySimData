...
RESULT_FILE="./tmpModes/idm_goffset_${GOFFSET}_results.txt"
...
for nt in 1 2 3 4; do
    ...
    logfile=$(ls -t ./tmpModes/fvdm_trainlog_*_1000_0_0_300_0.log 2>/dev/null | head -1)
    if [ -z "$logfile" ]; then
    ...
    set -- $res
    # 3) 结果行：屏幕显示 + 追加到文件
    printf "%-26s %-8.4f %-8.4f\n" "${nt}类fvdm跟车模型" "$1" "$2" | tee -a "$RESULT_FILE"
done
