...
def plot_param_boxplots(long_df, save_dir, tag=""):
    ...
    for p in long_df["param"].unique().tolist():
        ...
        fig.tight_layout()
        out = f"{save_dir}/boxplot_{p}.png" if not tag else f"{save_dir}/boxplot_{p}_{tag}.png"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--v0split', type=float, default=0.01, help='trainvalmode=2 时主车速度分割点(m/s)：训练集 v0<v0split，验证集 v0>v0split（开区间）')
