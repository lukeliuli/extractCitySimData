...
P.add_argument("--size", type=int, default=224, help="输入分辨率(单边)，6GB 显存建议 112/96")
...
# ===================== mini ResNet 模型 =====================
def build_mini_resnet(depth, size):
    """近似指定深度的 ResNet：1 卷积 + depth 个残差块前后。"""
    inpt = layers.Input((size, size, 3), name="input")
...
def estimate_flops(model):
    """粗估单样本前向 FLOPs（Conv/Dense 为主，忽略 Pool/Add/Activation）。
    反向约需 2 倍前向，故训练总 FLOPs ≈ 3×前向。用于归一化计算效率。"""
    fwd = 0
    for layer in model.layers:
        cls = type(layer).__name__
        out = layer.output_shape
        if cls == "Conv2D":
            h, w, c = out[1], out[2], out[3]
            kh, kw = layer.kernel_size
            in_c = layer.input_shape[-1]
            fwd += 2 * h * w * c * kh * kw * in_c
        elif cls == "Dense":
            fwd += 2 * out[-1] * layer.input_shape[-1]
        elif cls == "BatchNormalization":
            fwd += 4 * out[-1]
    return fwd * 3  # 前向 1 + 反向 2
...
def main():
    ...
    # 显存按需增长：避免 TF 一次性占满全部显存，与其他任务共存时不会直接互踢
    for g in gpus:
        try:
            tf.config.experimental.set_memory_growth(g, True)
        except Exception as e:
            print("  (set_memory_growth 失败，忽略:", e, ")")
    ...
    print("混精度:", "mixed_float16(TensorCore)" if args.mixed else "float32",
          "| 输入分辨率:", args.size)
    # 显存预检：直接读出当前占用，判断是否被其他进程占满
    try:
        out = subprocess.check_output(
            ["nvidia-smi", "--query-gpu=memory.used,memory.total",
             "--format=csv,noheader,nounits", "-i", args.gpu], text=True, timeout=5)
        used, total = [int(x.strip()) for x in out.strip().split(",")]
        print(f"当前显存: {used} MB / {total} MB",
              "| 已被占用" if used > total * 0.6 else "| 空闲充足")
    except Exception:
        print("当前显存: 无法查询(nvidia-smi 不可用)")
    print("=" * 72)
    ...
    for depth in depths:
        model = build_mini_resnet(depth, args.size)
        model.summary() if depth == depths[0] else None
        flops_sample = estimate_flops(model)
        ...
        for batch in batches:
            ...
            sampler.start()
            try:
                ds = make_data_source(batch, args.steps, mode="pipeline", size=args.size)
                t_pipe = bench_config(depth, batch, args.steps, args.warmup, ds, train_step)
                sampler.stop()
                s_pipe = sampler.mean()

                # 再测纯计算（缓存数据，排除数据加载/CPU影响）
                ds2 = make_data_source(batch, args.steps, mode="compute", size=args.size)
                t_comp = bench_config(depth, batch, args.steps, args.warmup, ds2, train_step)
            except Exception as e:
                sampler.stop()
                skipped.append(f"depth={depth} batch={batch}: {type(e).__name__}: {e}")
                print(f"!! 跳过 depth={depth} batch={batch} ({type(e).__name__})——继续下一配置")
                continue
            ...
            row = {
                ...
                # 实测达到的 TFLOPS 及占理论峰值的百分比（正确归一化口径）
                "achieved_tflops": round(samples_s * flops_sample / 1e12, 3),
                "compute_eff_pct": None,
            }
            if theo:
                row["compute_eff_pct"] = round(samples_s * flops_sample / theo / 1e12 * 100, 2)
            results.append(row)
    ...
    print("-" * 100)
    for r in results:
        e = f"{r['compute_eff_pct']}%" if r["compute_eff_pct"] is not None else "-"
    ...
    if theo:
        ...
        print(f"  最高吞吐: {hi['gpu']} depth={hi['depth']} batch={hi['batch']} = {hi['samples_per_sec']} samples/s"
              f" (实测 {hi['achieved_tflops']} TFLOPS, 占理论 {theo} TFLOPS 的 {hi['compute_eff_pct']}%)")
    ...
    if not low_util and not high_temp and not high_overhead:
        print("  未发现明显瓶颈指标。")
        if theo:
            print("  若出现'低端卡(1660Ti)比高端卡(2060)快'的观感，请对比两机的 compute_eff_pct：")
    ...
    if skipped:
        print("  [被跳过的配置]（多为 OOM，可降 --size 或减 batch 后重跑）：")
        for s in skipped:
            print("    -", s)
