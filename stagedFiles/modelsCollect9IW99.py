...
from tf_iw99_simulation import tf_iw99_simulation
...
BASE_BOUND_VEHICLE = [
    ...
    (0.1, 0.5),   # 7: CC7 (加速度波动) [m/s²]
    (1.0, 3.0),   # 8: CC8 (启动加速度) [m/s²]
    (0.5, 3.5),    # 9: CC9 (高速加速度) [m/s²]
    (0.9, 1.5)    # 10: lamba,天气因数，或者环境因素
]
...
def train_model_mlp_cf(X_train, y_train, raw_train, train_dataset, val_dataset, raw_cols, args, dt):
    ...
    output_dim = num_types2 * 11 # iW99需要10个参数
    ...
    def train_step(x_batch, y_batch, raw_batch):
        ...
        with tf.GradientTape() as tape:
            ...
            predicted_times = tf_iw99_simulation(
